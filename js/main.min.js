...
class App {
    ...
    try {
      ...
      const currentLocation =
        data.response.GeoObjectCollection.featureMember[0].GeoObject
          .description;
      // выбрать тренировку по айди мб и ей сделать значение geolocatinon отсюда
  ...
  isCorrect(type, distance, duration, cadence, elevation) {
    const isError = value => {
    ...
    if (type === 'running') {
      let errors = 0;
      if (isError(distance)) {
        if (!modalInfoWorkout.style.display === 'block')
  ...
  _newWorkout(e) {
    ...
    let workout;
    const answer = this.isCorrect(type, distance, duration, cadence, elevation);
    console.log(answer);
    if (answer === 'running')
      workout = new Running(distance, duration, [lat, lng], undefined, cadence);
    else if (answer === 'cycling')
      workout = new Cycling(distance, duration, [lat, lng], undefined, elevation);
    else {
      return;
    }
    this._getCoordinateAPI(lng, lat, workout);
    ...
    editForm.addEventListener('submit', e => {
      ...
      if (
        !this.isCorrect(
          ...
          currentWorkout.temp,
          currentWorkout.climb
        )
      )
        return this.areChangedPopup.call(this, false);
      this._editDataWorkout.call(this, index, currentWorkout);
    });
  }

  _showCity(workout) {
    cityName.innerHTML = `<p>Место: ${workout.geolocation}</p>`;
...
class Workout {
  ...
  constructor(distance, duration, coords, geolocation = 'Астрахань') {
...
class Running extends Workout {
  type = 'running';
  constructor(distance, duration, coords, geolocation, temp) {
    super(distance, duration, coords, geolocation);
    this.temp = temp;
    this.calculatePace();
  }
  calculatePace() {
...
class Cycling extends Workout {
  type = 'cycling';
  constructor(distance, duration, coords, geolocation, climb) {
    super(distance, duration, coords, geolocation);
    this.climb = climb;
    this.calculateSpeed();
  }
