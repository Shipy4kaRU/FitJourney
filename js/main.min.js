...
class App {
  ...
  _newWorkout(e) {
    ...
    if (type === 'running' && !inputCadence.value) {
      inputCadence.setCustomValidity('Поле не может быть пустым!');
      return inputCadence.reportValidity();
    }

    if (type === 'cycling' && !inputElevation.value) {
      inputElevation.setCustomValidity('Поле не может быть пустым!');
